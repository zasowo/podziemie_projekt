...
const editor = useEditor({
    ...
    extensions: [
      ...
      Image, // Tiptap's built-in Image
      Typography,
          Placeholder.configure({
        placeholder: 'Zacznij dodawanie strony...', 
    }),
      Superscript,
      Subscript,
      Selection,
      ImageUploadNode.configure({ // Your custom image upload node
        accept: "image/*",
      ...
      ImageResize // Tiptap extension for resizing images
    ],
    content: existingContent || '', // MODIFIED LINE
  })
...
<div className="content-wrapper bg-white border border-gray-300 rounded-md shadow-sm my-4"> {/* Added some basic Tailwind for wrapper */}
        <EditorContent
          editor={editor}
          role="presentation"
          className="simple-editor-content p-3 min-h-[300px]" // Added padding and min-height via Tailwind
        />
      </div>
      <div className="flex justify-end mt-4"> {/* Container for the button */}
        <button 
          onClick={handleSubmit} 
          disabled={isSubmitting}
          className="px-6 py-2.5 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:bg-gray-300 transition"
        >
          {isSubmitting ? 'Zapisywanie...' : (existingContent ? 'Zapisz zmiany' : 'Utwórz stronę')}
        </button>
      </div>
